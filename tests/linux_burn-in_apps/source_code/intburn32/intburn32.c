/*
 ...
 gcc intburn32.c cpuidc.c  int32.o -lrt -lc -lm -O3 -o intburn32
 ./intburn32
...
char    writeMsg1[256];
char    writeMsg2[256];
char    version[30] = "32 Bit Version 1.0";
...
int main(int argc, char *argv[])
{
    ...
    sprintf(errormsg, " ");
    xx = (unsigned int *)_mm_malloc((unsigned int)useMemK*1024+256, 16);
    if (xx == NULL)
    {
    ...
    for (j=6; j<12; j++) // 6 - 12
    {
        ...
        for (p=0; p<testPasses; p++)
        {
            ...
            printf(" L%2dP %3d Test%2d of 6, Pattern %8.8X, %4.1f Secs,  %6.0f MB/S\n",
                       testlog, p+1, j-5, printPattern, secs, (millionBytes * 2 * (double)n1 / secs));
        }
        results[j][0] = millionBytes * 2 * n1Count / timeCount;
        maxMIPS0 = n1Count * 102 * (double)mempasses / 1000000 / timeCount; // was 54 19 + 35
    ...
    printf("%s\n", errormsg);
    fprintf(outfile, " Max %6.0f 32 bit MIPS\n", maxMIPS1);
